...
- vm.nextTick和Vue.nextTick还可以作为**Promise**使用
...
- **:**  后的为传递的参数
  ...
  <!-- 动态特性名 (2.6.0+) -->
  <button v-bind:[key]="value"></button>    <!-- 此处的key可以理解为是一个变量 -->
...
- 在内联语句中使用**事件对象**时，可以利用特殊变量 **$event**:
...
预期值：**number** | **string** 有相同父元素的子元素必须有**独特的 key**，重复的 key 会造成渲染错误，key应**唯一**。
...
为了减少性能消耗。假如说，我们有一个计算属性A，它需要遍历一个巨大的数组并且做巨大的计算。然后我们需要使用到这个计算属性A，如果没有缓存，我们就会再次执行A的函数，这样性能开销就变得很大了。
...
当key值类型为**字符串**时，可以**实现监听对象当中的某一个属性**，如：
...
![image-20210108210846635](C:\Users\Ronin\AppData\Roaming\Typora\typora-user-images\image-20210108210846635.png)
...
**组件是可复用的Vue实例，且带有一个名字**，例如名字为shanshan-cmp，那么我们**则可以在一个通过new Vue创建的根实例中**，把这个组件作为自定义元素来使用：
...
利用Vue.component创建的组件组件是全局注册的。也就是说它们在**注册之后可以用在任何新创建的 Vue 根实例 (new Vue) 的模板中。**

参数：

- 参数一：**{string}**
- 参数二：**{Function | Object} [definition]**

用法： **注册**或**获取**全局组件。注册还会自动使用给定的id设置组件的名称。
...
Vue.component('button-counter', {//注册全局组件
  data () {
    return {
      count: 0,
    }
...
在Vue实例的**components**选项中定义要使用的组件。 对于 components 对象中的每一个属性来说，
...
取而代之的是，一个组件的 **data** 选项**必须是一个函数**，因此每个实例可以维护一份被返回对象的独立的拷贝：
（也就是是每一个组件的数据不受其他组件干扰）
...
![avatar](https://camo.githubusercontent.com/944be5b91f0a41b4bc494ffa66b278adb0d7cbc8dca1fe6ede291d22c5c15b75/68747470733a2f2f646576656c6f7065722e647579696564752e636f6d2f6d795675652f646174612e676966)
...
Vue.component('video-item', {
  props: ['title', 'poster', 'play', 'rank'], //注册的特性
})
```
...
Vue.component('video-item', {
  ...
  template: `<div>{{ title }}</div>` //访问数据
})
```

## Prop的大小写

HTML 中的**特性名是大小写不敏感的**，所以浏览器会把**所有大写字符**解释为**小写字符**。
故：当 传**递的prop为 短横线分隔命名时，组件内 的props 应为 驼峰命名** 。 如：

```js
<div id="app">
  <!-- 在 HTML 中是 kebab-case 的 -->//短横杠
...
<div class="app">
        <my-test
             v-for = "item in arrList" :key="item"
             :num = item
        ></my-test>
</div>
...
如果你想要将一个对象的所有属性都作为 prop 传入，你可以**使用不带参数的 v-bind** 。例如，对于一个给定的对象 person：
...
为了定制 prop 的验证方式，你可以为 props 中的值**提供一个带有验证需求的对象**，而不是一个字符串数组。例如：

```js
Vue.component('my-component', {
  props: {
    title: String, // 检查 prop 是否为给定的类型
    likes: Number,
    isPublished: Boolean,
    commentIds: Array,
    author: Object,
...
上述代码中，对prop进行了**基础的类型检查**，类型值可以为下列原生构造函数中的一种：
`String`、`Number`、`Boolean`、`Array`、`Object`、`Date`、`Function`、`Symbol`、任何自定义构造函数、或上述内容组成的**数组**。

 需要注意的是**`null`**  和  **`undefined`** 会通过任何类型验证。 除基础类型检查外，我们还可以配置高级选项，对prop进行其他验证，如：**类型检测**、**自定义验证**和**设置默认值**。 如：

```js
Vue.component('my-component', {
  props: {
    title: {
      type: String, // 检查 title 是否为给定的类型
      default: '杉杉最美',   // 为 title 指定一个默认值
      required: true, // 定义 title 是否是必填项
      validator (prop) {  // 自定义验证函数，title 的值会作为唯一的参数代入，若函数返回一个falsy的值，那么就代表验证失败
        return prop.length < 140; //返回false / true ，来表示验证 失败 / 通过
      }
    }
  }
})
...
**注意：** **对象**或**数组**的**默认值**必须从一个工厂**函数返回**，如：**default : ( ) => ( { a : 1 , b : 2 } )**

# 组件_单向数据流

所有的 prop 都使得其父子 prop 之间形成了一个**单向下行绑定**（也就是单项数据流）：
**父级 prop** 的**更新会向下流动到子组件**中，**但是反过来则不行**。
这样会防止从子组件意外改变父级组件的状态，从而导致你的应用的数据流向难以理解。

**注意：**也有特殊情况，就是传递的 prop 是一个**引用数据**，**那么子组件修改prop，也会影响父级的 prop**，因为他两都是指向同一个地址

这里有两种常见的试图改变一个 prop 的情形：

- **克隆方式**：这个 prop 用来传递一个**初始值**；
  这个子组件接下来希望将其作为一个本地的 prop 数据来使用，在后续操作中，会将这个值进行改变。在这种情况下，最好定义一个本地的 data 属性并将这个 prop 用作其初始值：
...
- **计算属性方式**：这个 prop 以一种**原始的值**传入且**需要进行转换**。
...
computed: {
  normalizedSize: function () {
    return this.size.trim().toLowerCase(); //去除空格 再转成小写
  }
}
```
...
**非Prop特性指的是**：一个**未被组件注册**的特性。当组件接收了一个非Prop特性时，该特性会被添加到这个组件的**根元素**上。
...
庆幸的是，class 和 style 特性会稍微智能一些，即两边的值会被**合并起来**，从而得到最终的值：**my-cmp  b**。
...
在这种情况下，非常适合去配合实例的 $attrs 属性使用，这个属性是一个**对象**，**键名**为**传递的特性名**，**键值**为**传递特性值**。

$attrs：是一个对象，里面存放的是模板中接收的特性
...
```js
<base-input label = "选择" type = "radio" value = "男神"></base-input>
...
Vue.component('base-input', {
  ...
  template: `
    <label>
      {{ label }}
      <input
        v-bind="$attrs"          //type = "radio"   $attrs = {type : "radio"}
        v-bind:value="value"     //value = "男神"
        v-on:input="$emit('input', $event.target.value)"   //$emit是用于触发父组件的自定义事件
      >
    </label>
  `,
})
...
**主要思想：通过在父组件监听一个事件，绑定触发函数，然后再通过子组件的 $emit 去触发该事件（冒泡）**

## 使用事件抛出一个值

在有些情况下，我们可能想让 `<blog-post>`组件决定它的文本要放大多少。这时可以**使用 $emit 的第二个参数来提供这个值**，如：
...
Vue.component('blog-post', {
  ...
  template: `
    <div class="blog-post">
      ...
      <button @click="$emit('enlarge-text', 0.2)">放大字号</button> <!-- 传递一个参数  0.2 -->
      ...
    </div>
  `,
})
...
```js
methods: {
  onEnlargeText: function (enlargeAmount) { //enlargeAmount = 0.2
...
如果触发一个camelCase名字的事件：（**也就是说，用驼峰命名的事件，监听时也必须是写驼峰命名，不能写短横杠表示）**
...
因此，推荐**始终使用 kebab-case 的事件名！始终使用 kebab-case 的事件名！始终使用 kebab-case 的事件名！**。

## 将原生事件绑定到组件

在组件上去**监听事件时（v-on）**，我们监听的是组件的自动触发的**自定义事件**，

但是在一些情況下，我们可能想要在一个组件的根元素上直接监听一个原生事件。

这时，可以使用 v-on 指令的 **.native** 修饰符，**作用就是监听原生事件（这样他就能通过子组件冒泡到父组件触发原生事件）**
...
不过在尝试监听一个类似`<input>`元素时，这并不是一个好主意，

例如`<base-input>`组件可能做了重构（也就是重新编写），如：
...
可以看到，此时组件的根元素实际上是一个元素，**那么父级的.native监听器将静默失败**。

它不会产生任何报错，但是**`onFocus`处理函数不会如预期被调用**。

为了解决这个问题，Vue提供了一个**$listeners**属性，它是一个**对象**，里面**包含了作用在这个组件上的所有监听器**。例如：
...
有了这个 **$listeners** 属性，我们**可以配合 v-on="$listeners"** 将所有的事件监听器**指向**这个组件的某个特定的**子元素**，如：
...
在 input 元素上使用v-mode指令时，相当于绑定了**value特性**以及**监听了input事件**：
...
>
<!-- $event.target.value：表示当前输入框的value -->
```
...
/>
<!-- $event 是子组件传递过来的参数，传递的是：$event.target.value -->
```
...
一个组件上的 v-model **默认**会利用名为 **value** 的 prop 和名为 **input** 的事件，

但是像单选框、复选框等类型的输入控件可能会将 value 特性用于不同的目的。

碰到这样的情况，我们可以利用 **model 选项**来避免冲突：
...
<div class="app">
        ...
        Vue.component("my-test",{
            props:["value"],
            model:{
                prop:"value",  <!-- 表示 特性 value 的值与 name 的值，进行双向绑定 -->
                event:"input"  <!-- 表示 默认绑定的事件是 input事件 -->
            },
            template:`
                <input type="text" :value = "value" @input = "$emit('input',$event.target.value)" />
            `
...
那么，该如何实现呢？ 先回忆一下，不利用 v-model 指令来实现组件的双向数据绑定：
...
那么，我们也可以试着，将监听的事件名进行更改（换成：“update:value”），如：
...
所以，.sync 修饰符 **与v-model一样**，本质上也是一个**语法糖**，在组件上使用：
...
- 带有.sync修饰符的v-bind指令，**只能**提供想要绑定的**属性名**，**不能**和表达式一起使用，如： **`:title.sync="1+1"`**，这样**操作是无效的**
- 将 `v-bind.sync` **用在 一个字面量对象上**，如 `v-bind.sync="{ title: 'haha' }"`，是**无法工作的**，因为在解析一个像这样的复杂表达式的时候，有很多边缘情况需要考虑。

## v-model VS .sync

先明确一件事情，在 vue 1.x 时，就已经支持 .sync 语法，但是此时的 .sync 可以完全在子组件中修改父组件的状态，造成整个状态的变换很难追溯，

所以官方在2.0时移除了这个特性。然后在 vue2.3时，.sync又回归了，跟以往不同的是，

现在的.sync完完全全就是一个**语法糖**的作用，**跟v-model的实现原理是一样的**，也不容易破环院有的数据模型，**所以使用上更安全也更方便。**
...
```html
<div>
  <slot></slot>     <!-- slot标签位置就是插槽的内容 -->
</div>
```

当组件渲染时，**`<slot></slot>`将会被替换为“写在组件标签结构中的内容**”。 

插槽内可以包含任何模板代码，包括**HTML**和**其他组件**。 如果`<my-cmp>`没有包含`<slot>`元素，则该组件起始标签和结束标签之间的任何内容都会被抛弃。
...
我们可以设置**默认插槽**，它会在没有提供内容时被渲染，如，在`<my-cmp>`组件中：
...
Vue.compopnent('my-cmp', {
  template: `
    <button type="submit">
      <slot>Submit</slot> <!-- Submit就是插槽的默认值，也就是当插槽没有内容时，就是用默认值 -->
    </button>
  `
})
```
...
此时，可以在**`<slot>`** 元素上使用一个**特殊的特性**：**name**。利用这个特性定义额外的插槽：
...
在向具名插槽提供内容的时候，我们可以在一个 **`<template>` 元素**上使用 **v-slot** 指令，并以 v-slot 的参数的形式提供其名称：

```html
<my-cmp>
  <template v-slot:header> <!-- 选择放到name为header的slot标签 -->
    <h1>头部</h1>
  </template>
...
**任何没有被包裹**在带有 **`v-slot`**的 **`<template>`**中的内容都**会被视为默认插槽**的内容。 
...
绑定在 `<slot>` 元素上的**特性**被称为**插槽 prop**。 

那么在父级作用域中，我们可以给**`v-slot`带一个值**来**定义**我们提供的**插槽prop的名字**：

```html
<div id="app">
  <my-cmp>
    <template v-slot:default="slotProps"> <!-- slotProps是随便定义的,将插槽prop的名字定义为slotProps -->
      {{ slotProps.user.name }}
    </template>
  </my-cmp>
</div>
...
```html
<my-cmp v-slot="{ user }"> <!-- 因为他接受的就是一个对象 -->
  {{ user.name }}
</my-cmp>
```
...
```html
<my-cmp>
  <template v-slot:[dynamicSlotName]> <!-- dynamicSlotName是一个变量 -->
    ...
  </template>
</my-cmp>
```
...
**语法：**使用标签 **component**，并设置属性 **is**

**is 的值**：**组件的名字**   /   **配置对象**（如：{ template : ‘<div>a</div>’}

```html
<div id="app">
	<component is="cmp-a"></component> <!-- is="cmp-a" 表示使用cmp-a组件 -->
</div>
Vue.component("cmp-a",{
	template:`
		<div>a</div>
...
通过上面方法，我们可以实现组件间的切换，能够注意到的是：
每一次切换标签时，都会**创建一个新的组件实例**，重新创建动态组件在更多情况下是非常有用的，但是在这个案例中，**我们会更希望哪些标签的组件实例能够在它们第一次被创建的时候缓存下来。**

为了解决这个问题，我们可以用一个**`<keep-alive>`元素**将动态组件包裹起来。如：
...
`<keep-alive>` 是一个**抽象组件**：它自身不会渲染一个 DOM 元素，也不会出现在父组件链中。 
...
接下来我们要学习的都是和处理边界情况有关的功能，即**一些需要对 Vue 的规则做一些小调整的特殊情况**。

需要注意的是，这些功能都是**有劣势**或**危险场景的**。

**也就是这些情况能不用就不用，除非自己可以控制好**
...
混入 (mixin) 提供了一种非常灵活的方式，来**分发 Vue 组件中的可复用功能**。
...
合并值为**对象**的选项，如 methods、components 等，**将被合并为同一个对象**。
...
Vue.mixin({  //通过这种方式混入，就会导致这个对象会混入到所有vue的实例对象上
  created () {
...
**注意**，在 Vue 中，**代码复用**和**抽象**的主要形式是**组件**。
...
Vue.directive('focus', {
  // 当被绑定的元素插入到DOM时执行  ，也就是页面加载完后，就会自动聚焦到输入框上
...
只调用一次，**指令第一次绑定到元素时调用**。（元素还未插入父节点,父节点也就是 vue实例绑定的元素，如：#app）
...
所在组件的 VNode也叫**虚拟节点**更新时调用（可以理解为：绑定的数据发生了改变时） ，**但是可能发生在其子 VNode 更新之前**。

如：  

```html
<!-- 当name发生改变时，就会触发update函数 -->
<input v-focus = "name" />
```
...
指令所在组件（或元素）的 VNode **及其子 VNode** 全部更新后调用。

### unbind

只调用一次，指令与元素**解绑**时调用(被绑定的Dom元素被Vue移除)。

## 钩子函数参数

- **el**: 指令所**绑定的元素**，可以用来直接操作DOM。当绑定的是**组件时**，**el**就是组件的**模板元素**
- **binding**：对象，包含以下属性：
  - **name**：指令名，不包括 v- 前缀。
...
- **vnode**：Vue 编译生成的虚拟节点。（里面有很多属性，一般只需要记住： **vnode.context 可以获取到父元素的vue实例**，如 vm）
...
当过滤器形式为  **`msg | filter`** 时，filter过滤器接收**一个参数**，参数为 **`msg`**。

当过滤器形式为  **`msg | filter('a')`**时，filter过滤器接收**两个参数**，参数为**`msg, 'a'`** 
...
在.vue文件所在目录下，使用命令： vue serve 文件名.后缀

默认（不写文件名）运行 目录下的 App.vue文件

**导入子组件：**

在App.vue中引入子组件 Son.vue
...
如果**不想**子组件中的style样式**污染**到全局的样式，可以在 style标签添加一个特性 ：**scoped**

**scoped的原理**：其实就是加上这个特性后，vue就会对该标签自动添加一个**随机的自定义特性**，然后渲染样式时他只会对带有该特性的元素进行渲染
...
- **第一轮选项**： 默认依赖  /    选择自定义依赖 【回车】

- （如果选择了自定义依赖）**第二轮选项**：一些相关的依赖，我们目前可以只选择Babel  ，选择使用【空格】，选完后【回车】

- **第三轮选项**：将依赖配置文件分开成单独文件  /    将依赖配置文件都放到  package.json 文件中 【回车】

- **第四轮选项**：是否将配置项保存为起来，y / n，y 的话就需要设置一个名字 【回车】

- 最后就是等待创建了

  
  创建完成后，项目文件夹的终端输入： npm run serve（具体其他的命令可以去 package.json 文件的 scripts中查看）
...
**template标签**   >    **render**    >      **template选项**      >       **el 、$mount**

所以，在使用render渲染函数时，不应该存在template标签
...
如果，我们按照之前的方式，那么模板内将会十分冗余。

如果此时利用**渲染函数**，那么代码写起来将会简洁很多。

render函数的语法： 
 **render( createElement ) { return  createElement( 标签名/组件名(必需), 与模板中属性对应的数据对象(可选), 子级虚拟节点(可选) ）}**

为了简洁，通常会把 **createElement** 用  **h**  代替
...
render: function (createElement) {
  return createElement(
    'h' + this.level,   // 标签名称
    this.$slots.default // 子节点数组，这里因为需要用到插槽，所以放的是 默认插槽
  )
},
```
...
当浏览器读到这些代码时，它会建立一个**DOM节点树** 来保持追踪所有内容，如同你会画一张家谱树来追踪家庭成员的发展一样。
 ...
 以上HTML对应的DOM节点树如下图所示： ![avatar](https://camo.githubusercontent.com/b8edf03cd1074158702b70d66acd9e51cfc3e826d41d113d40b2a9837eed1f5f/68747470733a2f2f636e2e7675656a732e6f72672f696d616765732f646f6d2d747265652e706e67)

每个**元素**都是一个节点。每段**文字**也是一个节点。甚至**注释**也都是节点。
...
Vue通过建立一个**虚拟DOM**来追踪自己要如何改变**真实DOM（也就是渲染到页面上的DOM）**。例如：
...
它不会返回一个实际的DOM元素，而是返回一个虚拟节点（实际上是一个对象）。
	    更准确的名字可能是：**`createNodeDescription`** ，因为**它所包含的信息会告诉Vue页面上需要渲染什么样的节点**，**包括其子节点的描述信息**。
...
```js
createElement(标签名/组件名(必需), 与模板中属性对应的数据对象(可选), 子级虚拟节点(可选));
```

使用时，如果不想写对象数据，可以用 { }表示，再写子级虚拟节点，如： createElement( “a”，{ }，“helloWorld ! ”)
...
```js
{
  ...
  // 普通的 HTML attribute，特性指的是写在HTML行间的值
  attrs: {
    id: 'foo',
  },
  // 组件 prop
  ...
  // DOM属性，也就是通过  DOM对象.属性名，来进行设置或读取的值
  domProps: {
    innerHTML: 'baz',
  },
...
**< 和 > 是用于书写HTML标签的，{ } 是用于书写JS代码的**

```html
<div>{ this.value }</div>
```

在 { } 中，可以直接放 存放着元素标签的**数组**，如： **[ <div>div</div>, <p>p</p>, template ]** ；

## 指令

在JSX中，**一些指令并不存在**，所以**我们可以换一种方式来处理**。

通过：**domProps属性名= 值**

来进行设置属性，如： domPropsInnerHTML = “hello”
...
<div class={{a: true, b: false}}>Content</div> <!-- 对象语法 -->
<div class={["a","b"]}>Content</div>  <!-- 数组语法 -->
<div style={{color: 'red', fontSize: '14px'}}>Content</div>   <!-- 对象语法 -->
```
...
会发现从 **this.$refs.xxx** 中获取的并**不是期望的数组值**，此时就需要将**refInFor属性设置为true**了：
...
当一个组件不需要状态（即响应式数据）、不需要任何生命周期场景、只接受一些props来显示组件时，
...
为了弥补缺少的实例，render 函数提供第二个参数context作为上下文。
...
- **children**: VNode 子节点的数组，包含了所有的**非作用域插槽**和**非具名插槽（也就是默认插槽）**。
...
**需要注意的是：**封装函数式组件时，style的样式不能使用 **scoped** 特性，否则样式作用不到外部调用的组件

# 过渡_单元素过渡

Vue 在**插入**、**更新**或者**移除** DOM 时，提供多种不同方式的应用过渡效果。
...
style里面设置的 v-enter 等类名，都会自动作用到transition标签里面
...
2. **v-enter-active**： 
   ...
   这个类可以被用来定义进入过渡的**过程时间**，**延迟**和**曲线函数**。如： transition : opacity 2s ;
3. **v-enter-to**: 
   定义**进入**过渡的**结束状态**(2.1.8+) 。 
...
3. **v-leave-to**: 
   定义**离开**过渡的**结束状态**(2.1.8+) 。 
   在离开过渡被触发之后下一帧生效 (与此同时 v-leave 被删除)，在过渡/动画完成之后移除。

------

图示： [![过渡](https://camo.githubusercontent.com/9047afc9cb2e4d95afa9c1a85b4c0d53fdf207a1fc8a6a253a909b83de40d68e/68747470733a2f2f636e2e7675656a732e6f72672f696d616765732f7472616e736974696f6e2e706e67)](https://camo.githubusercontent.com/9047afc9cb2e4d95afa9c1a85b4c0d53fdf207a1fc8a6a253a909b83de40d68e/68747470733a2f2f636e2e7675656a732e6f72672f696d616765732f7472616e736974696f6e2e706e67)
...
但是有些时候，我们会设置一系列的过渡效果，例如嵌套元素也有过渡动效，其过渡效果的时间长于父元素。

此时我们可以**设置duration属性**，**定制一个显性的过渡持续时间**（以毫秒记）：
...
- **before-enter**： 动画入场前（初始位置），可以在其中设置元素开始动画之前的起始样式

- **enter** ：动画入场中，可以在其中写动画

- **after-enter** ：动画完成后

- **enter-cancelled** ：取消动画（用的不多）

  **钩子函数会传一个参数 el** ：是表示模板的第一层元素

  **钩子函数第二个参数 done**：用于调用，来表示当前钩子函数的动画结束
  ...
  <!-- 这里 el 就是 <div>hello</div> -->
  ```

  **对于仅使用 JavaScript 过渡的元素**添加 **v-bind:css="false"**，Vue 会跳过 CSS 的检测。
  这也可以**避免过渡过程中 CSS 的影响**。
...
使用前需要先引入  import “velocity - animate”;

基本使用： **Velocity( 过渡元素的父级， { 过度之后的样式 } ，{ duration : .3s , complete : 回调函数 } ) ;**

# 过渡_多元素过渡

当切换展示的**元素标签名相同时**，**需要给每一个元素设置不同的key值**，否则Vue为了效率只会替换相同标签内部的内容（内容复用）。
...
Vue提供一个 mode 特性，可以给多个元素过渡应用不同的模式，
...
- **不同于 transition组件**，它会以一个**真实元素**呈现：**默认为一个 `<span>`**你也可以**通过 tag attribute 更换为其他元素**。
- **过渡模式不可用**，因为我们不再相互切换特有的元素。
- **内部元素** 总是需要 提供**唯一**的 **key** 属性值。
- **CSS 过渡的类**将会**应用在内部的元素**中，而不是这个组/容器本身。

这个组件中，元素的**新增**和**删除**都会**自动带有过渡效果**

**简单实例组件：渲染数组，并点击按钮随机位置添加数字**

```html
<template>
    <div>
        <button @click = "changeArr">add</button>
        <transition-group>
            <li
            v-for="num in list"
            :key="num"
...
export default {
    ...
    methods:{
        changeArr(){
            const num = parseInt(Math.random() * this.list.length);
            this.list.splice(num,0,this.n++);
        }
    }
}
</script>
...
`<transition-group>` 组件提供了一个新的特性：**v-move**，**它会在元素改变位置的过程中应用**。

 如何使用？

通过在css中的类名即可设置：**.v-move {}**。 
...
**注意**：当**移除**一个列表元素时，**需要将移除的元素脱离文档流**，否则，要溢出的元素在移除过渡中一直处于文档流中，会影响到后面元素的move过渡效果。

**内部的实现**：Vue 使用了一个叫 FLIP 简单的动画队列，使用 transforms 将元素从之前的位置平滑过渡新的位置。

**注意**：使用 FLIP 过渡的元素**不能设置为 display: inline** 。作为替代方案，**可以设置为 display: inline-block** 或者**放置于 flex 中**。
...
**注意**：当使用函数式组件复用过渡时，不要设置css作用域（scoped）。

**封装一个简单的函数式过度组件：** 使用方法就是通过导入该组件，然后把需要使用过度的元素放进组件默认插槽内即可
...
export default {
    functional:true,
    render(h,context){
        const { slots } = context; //在context中拿到作用域插槽的内容
        //slots().default 里面存放着默认插槽的元素标签数组
        return(
            <transition name="single"> 
            //设置一个name ，避免跟其他的组件冲突，注意：设置name后，类名就不是 v-，而是 name-
                { slots().default }
            </transition>
        )
    }
...
异步加载的文件，会在引入合并后js文件的**link**标签上设置 **el="prefech"**（预加载）。

浏览器会在**空闲时间内下载对应的资源**，**使用时**可以直接**从缓存中获取**。（**有利于使用的时候更快速的加载**）

与之对应的 **el="preload"**，会**立刻下载对应的资源**（也就是有这个特性的link标签，会直接下载资源，不会等到空闲时间）。

# **VueRouter_基础**

## 什么是路由？

路由是**根据不同的url地址展现不同的内容或页面**。 

早期的路由都是后端直接根据url来重载页面实现的，即后端控制路由。 后来页面越来越复杂，服务器压力越来越大，随着ajax（异步刷新技术）的出现，页面的实现非重载就能刷新数据，让前端也可以控制url自行管理，前端路由由此而生。
...
<div id="app">
  ...
  <p>
    ...
    <!-- 通过传入 `to` 属性指定路径. -->
    <!-- <router-link> 默认会被渲染成一个 `<a>` 标签 ，可以通过 tag属性设置为其他标签-->
...
**通常会把这些路由配置放到一个JS文件中，然后只需要在main.js文件中引入 router，然后将router挂载到实例上就行**
...
const routes = [
    ...
    //为了可以节省性能消耗，可以将首页之外的组件使用异步组件
    {
        path:"/search",
        component:()=>import("./component/Search.vue");
    }
]
//导出VueRouter的实例对象，然后在main.js中将这个对象与vue实例挂在起来即可
...
**还有一点就是：**一般这种页面级的组件，习惯把他们放到一个 views文件夹下 

## router-link class

- **router-link-exact-active** ：当前展示路径完全匹配组件to属性的值（也就是router-link被选中）
- **router-link-active** ：当前展示路径包含to属性的值

  可以利用这些类名，来设置被选中后的router-link的样式
...
**可以通过一个名称标识一个路由**，这样在某些时候会显得更方便一些，

特别是在链接一个路由，或者是执行一些跳转时，可以在创建Router实例时，在routes配置中给某个路由设置名称：

```js
routes = [
  {
    path: '/activity/personal',
    name: 'personal', //配置name
    component: Personal,
  }
];
```
...
这样可以避免遇到一些很长的路径，有了name后，就能更方便的使用组件
...
```js
{
  ...
  children: [
    {
      path: '/activity/academic',  //路径可以简写为： academic，注意：这里不能在前面加 / ，他会在根路径进行查找
      name: 'academic',
...
**当访问 /activity 下的其他路径时，并不会渲染出来任何东西**，如果想要渲染点什么，可以提供一个**空路由**：

这样是为了当访问activity页面时，他就会显示出空路由的内容，如果没有空路由，那么只有点击了 router-link才会显示内容
...
**重定向目的**：就是让你进入这个组件后，它会立刻重定向 进到该组件的 一个子组件中，否则内容需要点击进入才会显示

**其实就是相当于代替了 空路由**

重定向也是通过 routes 配置来完成，

下面例子是从 /a 重定向到 a组件下的 b组件

通过设置 **redirect : 子路由路径** 

**需要注意的是，配置了重定向后，就不能存在空路由了**
...
这个方法**会向 history 栈添加一个新的记录**，所以，**当用户点击浏览器后退按钮时，则回到之前的 URL**。
...
**注意：该方法如果导航的URL是与当前URL相同，则会报错**

### $router.replace

跟 router.push 很像，唯一的不同就是，**它不会向 history 添加新记录**，而是**替换掉当前的 history 记录**。
...
```html
<router-link :to="{name:'home',params:{'id':'123'}}" tag="div">点击跳转home</router-link>
<!-- 跳转后，就会传递 params里面的值到  $route.params -->
<!-- 可以在函数中用  this.$route.params获取 -->
```
...
经过这样的设置，像 /user/foo 和 /user/bar **都将映射到相同的路由**。（也就是指，user后面接什么路径都是隐射相同路由）
...
想**同时展示多个视图时**，并且**每个视图展示不同的组件**时，可以**使用命名视图**。

可以在界面中拥有多个**单独命名**的视图，而不是只有一个单独的出口。
...
接收**参数方**：是跳转到的路由组件 （也就是 name 为 home 的路由组件）
...
**配合动态路由使用时，因为它是一直都是在当前路由的，只是改变了id值，所以可以在自身路由组件获取属性props，它就能通过点击router-link 切换id，然后实现其他不同的操作**

### 布尔模式

如果 props 被设置为 true，**route.params** 将会被设置为组件属性。

如： $route.params = { id : 123 };

那么，他就会将 id 作为属性名， 123 作为属性值，进行传递
...
**导航守卫**被分成三种：**全局的**、**单个路由独享的**、**组件内的**。

守卫都是在 **router.js配置文件** 中设置的

## 全局守卫

是指**路由实例上直接操作**的**钩子函数**，触发路由就会触发这些钩子函数。
...
- **to**： **目标路由**对象（this.$route）
- **from**： **即将要离开**的路由对象
- **next**： 三个参数中最重要的参数。
  - **必须调用next()**，才能继续往下执行一个钩子，**否则路由跳转会停止**
  - 若要**中断**当前的导航，可以调用**next(false)**。（也就是传一个参数为false）
  - 可以使用next**跳转**到一个不同的地址。中断当前导航，进入一个新的导航。**next参数值**和**$routet.push**一致。
  - **next(error)**。2.4+，如果传入的参数是一个**Error实例**，则导航**会被终止**，且该错误会被传递给router.onError() 注册过的回调。

### 全局解析守卫 beforeResolve

和boforeEach类似，**路由跳转前触发**。但是稍微**比 beforeEach 要慢一些**

**和beforeEach的区别**：在导航被确认之前，同时在所有组件内，**守卫**和**异步路由组件**被解析**之后**，解析守卫就被调用。
...
是指在**单个路由**配置的时候**也可以设置的钩子函数**。

**简单理解**：就是只有A路由去到B路由才会触发守卫，其他的都不影响

**注意：** 肯定是先经过全局守卫后，再经过独享守卫的（就好比 进入以家商场，必须先经过大门，之后再进入商店门）
...
是指在**组件内执行的钩子函数**，**类似于组件内的生命周期**，相当于**为配置路由的组件添加的生命周期钩子函数**。

### beforeRouteEnter

**路由进入之前调用**。

在该守卫内**访问不到组件的实例**，**this值为undefined**。

在这个钩子函数中，**可以通过传一个回调给 next来访问组件实例**。

在导航被确认的时候执行回调，并且**把组件实例作为回调方法的参数**，**可以在这个守卫中请求服务端获取数据**，当成功获取并能进入路由时，调用next并在回调中通过 vm访问组件实例进行赋值等操作，
...
在**当前路由改变时**，并且**该组件被复用时**调用，可以通过this访问实例。

（也就是路径发生改变时）

何时组件会被复用？

- **动态路由间互相跳转**（跳转时路径改变了，也就是向 $route.params传参了）
- **路由query变更**
...
1. 导航被触发。
2. 在失活的组件（**A组件**）里调用离开守卫。（beforeRouteLeave）
3. 调用**全局**的 beforeEach 守卫。（因为全局守卫是守卫全局的路由，也就是所有路由都守卫）
4. （**如果是动态路由**）在重用的组件里调用 beforeRouteUpdate 守卫 (2.2+)。
5. （**A组件**）在路由配置里调用 beforeEnter。
6. 解析异步路由组件。（**如果B组件是异步组件**）
7. 在被激活的组件（**B组件**）里调用 beforeRouteEnter。
8. 调用**全局**的 beforeResolve 守卫 (2.5+)。（跳转前）
9. 导航被确认。（此时才**开始跳转**）
10. 调用全局的 afterEach 钩子。（**跳转后**）
11. 触发 DOM 更新。（**执行组件里面的生命周期钩子函数**，如：beforeCreate、created等等）
12. 用创建好的实例**调用** beforeRouteEnter 守卫中传给 **next 的回调函数**。
...
- 用于验证是否登录：用一个自定义属性存储 true / false，通过判定决定是否登录
...
router-view是基本的动态组件，所以我们可以用 组件给它添加一些过渡效果。
...
**适合共享状态（数据）比较多，比较大型的项目使用**

Vuex是vue的状态（数据）管理工具，为了更方便的实现多个组件共享状态（数据）。
...
Vuex 通过store 选项，提供了一种机制将状态从根组件“**注入**”到**每一个子组件**中（**调用Vue.use(Vuex)**）。

通过在根实例中注册store选项，该store实例会注入到根组件下的所有子组件中，且子组件能通过**this.$store**访问。
...
mapState函数返回一个**对象**，里面就是我们要生成计算属性，所以我们只需要把他**展开**放进去即可

```js
import { mapState } from 'vuex';

computed: {
  ...mapState(['count']), // 会自动创建 count(){ return this.$store.count}
},
```

**为了防止与 组件data中的数据重名**（如果重名，会被data覆盖，data优先级较高）
...
computed: {
  ...mapState({
    storeCount: state => state.count,  //变量名 :  返回变量值
    // 简写
...
store的**计算属性**。**getters**的返回值会根据它的依赖被缓存起来，且只有当它的依赖值发生了改变才会被重新计算。

Getter 接收state作为其第一个参数、**getters**作为其第二个参数。
...
const store = new Vuex.Store({
  ...
  getters: {
  	doubleCount (state) {  //第一个参数 state，第二个参数 getters
...
```js
getters: {
  addCount: state => num => state.count + num;  //变量 num
}
...
自动将 this.$store 的值放到 computed 中
...
如果在Vuex的严格模式下**更改** Vuex 的 **store 中的状态的唯一方法**是**提交 mutation**。

Vuex的严格模式开启方式：创建Store对象时，传一个配置项 **strict : ture**
...
<img src="C:\Users\Ronin\AppData\Roaming\Typora\typora-user-images\image-20210121134355198.png" alt="image-20210121134355198" style="zoom:50%;" />

不能直接调用一个mutation handler。这个选项更像是事件注册：“当触发一个类型为`countIncrement`的mutation时，调用次函数。”：
...
你可以向**store.commit**传入**额外的参数**，即mutation的载荷（**payload**）：
...
**提交 mutation** 的另一种方式是**直接使用包含 type 属性的对象**：
...
当使用对象风格的提交方式，**整个对象都作为载荷传给 mutation 函数**，因此与上面的传递对象一样效果：
...
把这些常量放在**单独的文件**中并**导出**，可以让你的代码合作者对整个 app 包含的 mutation 一目了然：

```js
// mutation-types.js文件
export const COUNT_INCREMENT = 'COUNT_INCREMENT' //导出一个常量
...
const store = new Vuex.Store({
  state: { ... },
  mutations: {
    [COUNT_INCREMENT] (state) {  //把常量名设置为mutations类型名
      // ...
    }
  }
})
...
- **当需要在对象上添加新属性时**，你应该：
  ...
  - **以新对象替换老对象**。
**使用这些方法来设置，是因为在vue中直接修改引用值不会是页面重新渲染**
...
如果想要使用双向数据的功能，就**需要自己模拟一个v-model: :value="msg" @input="updateMsg"**。
...
因为在异步函数中，如下setTimeout函数，它会在window环境执行函数，这样就**相当于在mutation之外**修改数据了，就会导致报错
...
执行上段代码，我们会发现更改state的操作是在回调函数中执行的，这样会**让我们的代码在devtools中变的不好调试**：

当 mutation 触发的时候，回调函数还没有被调用，devtools 不知道什么时候回调函数实际上被调用，**任何在回调函数中进行状态的改变都是不可追踪的。**
...
因此你可以**调用 context.commit 提交一个 mutation**，

或者**通过 context.state** 和 **context.getters** 来**获取 state 和 getters:**
...
const store = new Vuex.Store({
  ...
  mutations: {
    increment (state,{num}) { //{num} 是展开对象并 设置num参数
      state.count += num;
    }
  },
  actions: {
    increment (context,payload) { //payload为接收的参数num
      context.commit('increment',payload) //提交mutation ，然后执行mutations的 increment函数：state.count+= 10;
    }
  }
})
...
methods:{
    changeClick(){
        const num = 10;
        this.$store.dispatch("incrementAsync",{num});  //传递的是一个对象{num}
    }
}
```

Action其实也有辅助函数  mapAction ，其用法和上面的 mapMutation 这些完全一样
...
export default {
    ...
    getters:{
        changeCount1:state=>state.count1 * 2;
    },
    mutations:{
        count1_increment(state,{num}){
            state.count1 += num;
        }
    },
    actions:{
        count1_increment(context,payload){
            context.commit("count1_increment",payload);
        }
    }
}
//b.js 懒得写了
...
- 可以通过**mapXXX**的方式**拿到getters、mutations、actions**，但是**不能拿到state**，
  如果想通过这种方式**获得state**，**需要加命名空间**。

## 命名空间

可以通过添加 **namespaced: true** 的方式使其成为**带命名空间的模块**。

- **获取 state**：this.$store.state.**moduleName**.xxx
- **获取 getter**：this.$store.['**moduleName**/getters'].xxx   （注意是 moduleName/ ，不是或）
- **提交 mutation**：this.$store.commit('**moduleName**/xxx');
- **分发 action**：this.$store.dispatch('**moduleName**/xxx');
- 可以通过**mapXXX的方式**获取到**state、getters、mutations、actions**。

## 模块的局部状态的获取

对于模块内部的 **mutation** 和 **getter**，接收的**第一个参数**是模块的**局部状态对象**。

同样，对于模块内部的 **action**，**局部状态**通过 **context.state** 暴露出来，**根节点状态**则为 **context.rootState**。

对于模块内部的 **getter**，**根节点状态**会作为**第三个参数**暴露出来。